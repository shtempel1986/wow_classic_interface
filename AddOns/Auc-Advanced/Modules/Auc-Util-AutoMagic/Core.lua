--[[
	...
	Revision: $Id: Core.lua 6940 2023-08-28 00:00:20Z none $
	...
	for bag=0,4 do
		for slot=1,GetContainerNumSlots(bag) do
			if (GetContainerItemLink(bag,slot)) then
				...
				local texture, itemCount, locked, _, lootable = GetContainerItemInfoSpecial(bag, slot) --items that have been vedored but are still in players bag (lag) will be locked by server.
	...
	for bag=0,4 do
		for slot=1,GetContainerNumSlots(bag) do
			if (GetContainerItemLink(bag,slot)) then
				...
				if (itemLink == nil) then return end
				if itemCount == nil then _, itemCount = GetContainerItemInfoSpecial(bag, slot) end
				if itemCount == nil then itemCount = 1 end
				local linkType, itemID, _, _, _, _ = decode(itemLink)
				if linkType == "item" then
...
AucAdvanced.RegisterRevision("$URL: Auc-Advanced/Modules/Auc-Util-AutoMagic/Core.lua $", "$Rev: 6940 $")
