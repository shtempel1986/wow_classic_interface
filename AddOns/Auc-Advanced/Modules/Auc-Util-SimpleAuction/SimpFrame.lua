--[[
	...
	Version: 3.4.6963 (SwimmingSeadragon)
	Revision: $Id: SimpFrame.lua 6963 2023-08-28 00:00:20Z none $
...
function private.LoadItemLink(itemLink, size)
	...
	if itemLink then
		...
		if not texture and frame.icon.ClearNormalTexture then -- ### hybrid
		...
		if frame.icon.ClearNormalTexture then -- ### hybrid
			frame.icon:ClearNormalTexture()
		else
...
function private.CreateFrames()
	...
	frame.scanbutton:SetWidth(100)
	frame.scanbutton:SetHeight(15)
...
AucAdvanced.RegisterRevision("$URL: Auc-Advanced/Modules/Auc-Util-SimpleAuction/SimpFrame.lua $", "$Rev: 6963 $")
